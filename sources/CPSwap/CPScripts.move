...
module cp_scripts {
    ...
    public fun create_new_pool<X, Y>(
        ...
        assert!(coin_list::is_registry_initialized(), error::invalid_state(E_TOKEN_REGISTRY_NOT_INITIALIZED));

        assert!(coin_list::is_coin_registered<X>(), error::invalid_state(E_TOKEN_X_NOT_REGISTERED));
        assert!(coin_list::is_coin_registered<Y>(), error::invalid_state(E_TOKEN_Y_NOT_REGISTERED));

        assert!(!coin_list::is_coin_registered<cp_swap::LPToken<X, Y>>(), error::invalid_state(E_LP_TOKEN_ALREADY_REGISTERED));
        assert!(!coin_list::is_coin_registered<cp_swap::LPToken<Y, X>>(), error::invalid_state(E_LP_TOKEN_ALREADY_REGISTERED));

        assert!(!coin_list::is_coin_in_list<cp_swap::LPToken<X, Y>>(admin_addr), error::invalid_state(E_LP_TOKEN_ALREADY_IN_COIN_LIST));
        assert!(!coin_list::is_coin_in_list<cp_swap::LPToken<Y, X>>(admin_addr), error::invalid_state(E_LP_TOKEN_ALREADY_IN_COIN_LIST));
    ...
    public entry fun swap_script<X, Y>(
        ...
        assert!(!(x_in > 0 && y_in > 0), error::invalid_argument(E_SWAP_ONLY_ONE_IN_ALLOWED));
        assert!(!(x_min_out > 0 && y_min_out > 0), error::invalid_argument(E_SWAP_ONLY_ONE_OUT_ALLOWED));
        // X to Y
        if (x_in > 0) {
            let y_out = cp_swap::swap_x_to_exact_y<X, Y>(sender, x_in, signer::address_of(sender));
            assert!(y_out >= y_min_out, error::invalid_argument(E_OUTPUT_LESS_THAN_MIN));
        }else if (y_in > 0) {
            let x_out = cp_swap::swap_y_to_exact_x<X, Y>(sender, y_in, signer::address_of(sender));
            assert!(x_out >= x_min_out, error::invalid_argument(E_OUTPUT_LESS_THAN_MIN));
        }else {
            assert!(false, error::invalid_argument(E_SWAP_NONZERO_INPUT_REQUIRED));
        }
    }
